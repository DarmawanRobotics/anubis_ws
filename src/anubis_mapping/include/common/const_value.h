...
namespace anubis_mapping
{
    ...
    // IMU samples arrive at roughly 200 Hz on Mid-360.  The old value (20)
    // completed initialization after only about 0.1 s, which is too short to
    // average startup vibration.  The time-based gate in ImuProcess is the
    // normal completion criterion; this is only a finite sample-count guard.
    constexpr int MAX_INI_COUNT = 1000;
